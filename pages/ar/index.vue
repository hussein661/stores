...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      const language = 'ar/'
      const url = `https://api.storyblok.com/v1/cdn/stories/?starts_with=${language}stores%2F&version=published&cv=${ts}&token=YPJhIqzQepWQtUNNSVIPXAtt&per_page=100&sort_by=position:asc&clear=auto`;
  ...
  computed: {
    hala() {
      return false ? "Hala" : "هلا";
    },
    by() {
      return false ? "by" : "من";
    },
    menasa() {
      return false ? "Menasa" : "منصة";
    },
    addstitle(){
      return false ? 
